...
Multiple sclerosis (MS) is a neurological disease in which the immune system attacks the myelin
sheath in the brain and spinal cord, leaving visible scars called lesions on MRI scans. Neurologists
monitor disease progression by counting and measuring those lesions on each new scan, a process that
can take 20-40 minutes per patient and where two radiologists do not always agree exactly. This
project trains a neural network to perform that same delineation automatically, learning from
examples previously annotated by experts.

**In brief**: a 2D U-Net trained on brain MRI (T1/T2/FLAIR) from 60 MS patients, with
patient-level 5-fold cross-validation. A U-Net is a type of neural network designed specifically
for medical image segmentation: it learns to identify visual patterns from thousands of annotated
examples and then applies that knowledge to new images.
...
*Each row is one axial brain slice. Left: MRI image (FLAIR channel). Center: radiologist annotation
(lesion in white). Right: model prediction (lesion in white). Areas where both overlap are correctly
detected lesions.*

![Training and validation loss/Dice curves per fold](outputs/figures/training_curves.png)

*Loss and Dice curves during training, one line per fold. Solid lines are the training set; dashed
lines are validation. The fast drop at the start is normal -- the model learns the most obvious
patterns first. Divergence at the end of some folds marks where early stopping halted training to
prevent overfitting.*
...
> **What does Dice = 0.563 mean?** The Dice coefficient measures the overlap between the model's
> prediction and the radiologist's annotation: 1.0 is perfect agreement, 0.0 means nothing was
> detected. A value of 0.563 means the model and the specialist agree on roughly 56% of the lesion
> area. That is enough to be useful as a screening aid, but not to replace clinical review. For
> context, inter-rater variability between human radiologists on this task typically yields Dice
> scores of 0.60-0.70.

Best result from 6 training experiments (see [`outputs/experiments.md`](outputs/experiments.md)).
Note: a preprocessing axis bug (nibabel vs. SimpleITK convention mismatch) was discovered and fixed
after Exp 5; the numbers above reflect the corrected retraining.

## What we found

The best model (Exp 4) uses Tversky loss with alpha=0.6, which penalizes missed lesions more than
false positives. This raised sensitivity by +0.048 over the baseline, meaning the model correctly
identifies more of the actual lesion area, while keeping precision high enough to avoid flooding
predictions with false alarms. Adding a learning rate scheduler and early stopping prevented
overfitting and stabilized results across folds.

**Two experiments that did not work, and why they are interesting:**

- *Anatomical registration* (Exp 5, Dice 0.132): replacing simple image rescaling with proper 3D
  registration collapsed performance. The reason: this dataset's MRI files have placeholder
  coordinate headers (all zeros), so the registration optimizer had no real patient-space
  information to work with. Lesson: anatomical registration only helps when NIfTI files contain
  real scanner coordinates.
- *Intensity augmentation* (Exp 6, Dice 0.175): randomly distorting image contrast during training
  made things worse, not better. The distortion range was too wide -- training images looked so
  different from validation images that the model learned the wrong patterns. Lesson: data
  augmentation needs to stay close enough to the real distribution.

**The persistent outlier (fold 1):** one validation fold consistently scores ~0.07 lower than the
others. Investigation shows this fold happens to contain two patients with unusually tiny lesions
(~4x5 pixel blobs at 256x256 resolution), which are near-impossible to detect reliably regardless
of the model. This is a data split artifact, not a model failure.

**Next steps (prioritized):**

1. Intensity augmentation with conservative ranges (gamma +-10%, brightness +-5%) -- same idea as
   Exp 6, but without the distribution mismatch that caused the regression.
2. 2.5D architecture -- feed adjacent slices as extra input channels to give the model spatial
   context across slices, which should help with small lesions that only appear in 1-2 consecutive
   slices.
3. Stratified split -- distribute the hard small-lesion patients evenly across folds so per-fold
   variance better reflects model quality rather than data luck.
...
`src/data/download.py` uses the `kaggle` CLI (v2.x), which needs an API token. Kaggle's current
token format (`KGAT_...`) is read from an `access_token` file, resolved via `KAGGLE_CONFIG_DIR`
(defaults to `~/.kaggle`).
...
(The classic `kaggle.json` with `KAGGLE_USERNAME`/`KAGGLE_KEY` still works too, if you have one
from an older token.)
...
- `summary.json`: slice counts, total lesion voxels, Dice (only if ground-truth mask is present).

The script applies the same N4 bias correction and FLAIR-space resampling as the training
preprocessing. If no ground-truth mask is found in the patient folder, it runs in prediction-only
mode without computing Dice.
...
- **Modalities**: Each MRI scan comes in three sequences that highlight different tissue properties.
  ...
  space, then resamples T1/T2 onto the FLAIR grid via SimpleITK rigid registration (Euler3D
...
- Smallest lesions (~4x5 pixels at 256x256) are near the detection limit of any standard conv model.
